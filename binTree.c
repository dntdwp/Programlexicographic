...
// ==============================================
// FUNGSI UNTUK LINKED LIST SINONIM
// ==============================================

// Bikin salinan dari daftar sinonim untuk copy node tree
addrSinonim CopySinonimList(addrSinonim S) {
  if (S == NULL)
    return NULL;
  addrSinonim newHead = (addrSinonim)malloc(sizeof(NodeSinonim));
  strcpy(newHead->kata, S->kata);
  newHead->next = CopySinonimList(S->next);
  return newHead;
}

// membersihkan memori saat node dihapus
...
// Cetak rentetan kata sinonim dipisah koma
...
boolean IsEmptyT(BinTree P) { return (P == Nil); }

void CreateEmpty(BinTree *P) { *P = Nil; }
...
void BuildTree(BinTree *P) {
  ...
  if (strcmp(X, ".") == 0) {
    *P = Nil;
    ...
    addrSinonim newS = (addrSinonim)malloc(sizeof(NodeSinonim));
    strcpy(newS->kata, tempS);
    newS->next = NULL;

    *P = AlokasiTree(X, newS);
    if (*P != Nil) {
...
int NbDaun(BinTree P) {
  ...
  // Jika sebuah node tidak punya anak kiri DAN tidak punya anak kanan, berarti
  // dia daun
...
boolean IsSkewLeft(BinTree P) {
  /* Mengirimkan true jika P adalah pohon condong kiri (semua node hanya punya
   * anak kiri) */
  if (IsEmptyT(P)) {
...
boolean IsSkewRight(BinTree P) {
  /* Mengirimkan true jika P adalah pohon condong kanan (semua node hanya punya
   * anak kanan) */
  if (IsEmptyT(P)) {
...
void AddDaun(BinTree *P, infotype X, infotype Y, addrSinonim SY,
             boolean InputKiri) {
  ...
  // Note: jika tidak ketemu sampai daun, memori SY tidak ter-free. Ini adalah
  // resiko fungsi yg sangat jarang digunakan.
...
ListOfNode MakeListDaun(BinTree P) {
  ...
  // Jika ketemu daun, buat node list baru dengan cabang kiri dan kanan diset
  // Nil
  if (Left(P) == Nil && Right(P) == Nil) {
    addressT nodeBaru = AlokasiTree(Info(P), CopySinonimList(Sinonim(P)));
    return nodeBaru;
  }
  ...
  // Gabungkan list kiri dan list kanan (sambungkan ekor list kiri ke kepala
  // list kanan)
  if (leftList == Nil) {
    return rightList;
  } else {
...
ListOfNode MakeListPreoder(BinTree P) {
  ...
  addressT akarList = AlokasiTree(Info(P), CopySinonimList(Sinonim(P)));
...
ListOfNode MakeListLevel(BinTree P, int N) {
  ...
  /* Jika P bukan pohon kosong, menghasilkan list yang elemennya adalah semua
   * elemen pohon P yang levelnya N */
  if (IsEmptyT(P) || N < 1) {
    return Nil;
  }

  // Kondisi dasar: Jika sudah mencapai level target (N == 1 karena akar dimulai
  // dari level 1)
  if (N == 1) {
    return AlokasiTree(Info(P), CopySinonimList(Sinonim(P)));
  }
...
BinTree BuildBalanceTree(int n) {
  ...
  if (n <= 0) {
    ...
    P = AlokasiTree(X, newS);
    if (P != Nil) {
      // 4. Bangun subpohon kiri dan kanan secara rekursif sesuai jatah nL dan
      // nR
...
BinTree InsSearch(BinTree *P, infotype X, addrSinonim S) {
  ...
  if (strcmp(X, Info(*P)) < 0) {
    ...
    // Kata sudah ada. List S yang dikirim dari pemanggil awal bisa di-free
    FreeSinonim(S);
  }
...
void DelBTree(BinTree *P, infotype X) {
  ...
  // 2. Jika node X sudah ditemukan (Info(*P) == X)
    ...
    // Kasus 3: Node punya 2 anak (Biner murni)
      ...
      // Cari kandidat pengganti dari subpohon kanan yang paling kiri (nilai
      // terkecil di kanan)
      ...
      FreeSinonim(Sinonim(*P)); // free list lama
      Sinonim(*P) = CopySinonimList(Sinonim(successor));
...
void isiBST(const char *FILENAME,
            BinTree *KAMUS) { // ngisi bst pake kamus yang ada
  char line[1024];
  infotype kata;
  char bufSinonim[1024];
  ...
  while (fgets(line, sizeof(line), f) != NULL) {
    ...
    while (*afterComma == ' ')
      afterComma++; // trim spasi depan

    i = 0;
    while (afterComma[i] != '\0' && afterComma[i] != '\n' &&
           afterComma[i] != '\r' && i < 1000) {
      bufSinonim[i] = afterComma[i];
      i++;
    }
    bufSinonim[i] = '\0';

    // Trim spasi belakang dari sinonim
    int len = strlen(bufSinonim);
    while (len > 0 && bufSinonim[len - 1] == ' ') {
      bufSinonim[len - 1] = '\0';
      len--;
    }

    if (strlen(bufSinonim) == 0)
      continue;

    // Parse sinonim jadi linked list
    addrSinonim headS = NULL;
    addrSinonim tailS = NULL;

    char *token = strtok(bufSinonim, ",");
    while (token != NULL) {
      // trim depan
      while (*token == ' ')
        token++;
      ...
      while (tlen > 0 && (token[tlen - 1] == ' ' || token[tlen - 1] == '\r' ||
                          token[tlen - 1] == '\n')) {
        token[tlen - 1] = '\0';
        tlen--;
      }
      ...
      if (strlen(token) > 0) {
        addrSinonim newNode = (addrSinonim)malloc(sizeof(NodeSinonim));
        strcpy(newNode->kata, token);
        newNode->next = NULL;
...
void CariSinonim(BinTree P, char kata[31]) {
  ...
  int compare = strcmp(kata, Info(P));
  if (compare == 0) {
    printf("Kata    : %s\n", Info(P));
    printf("Sinonim : ");
    PrintSinonim(Sinonim(P));
